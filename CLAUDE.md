...
- **⚠ EVERY OPERATIONAL INSTRUCTION IN THE FOUR LINES ABOVE IS NOW WRONG.
  ...
  applies and running from main is correct; the window is **7d**, not 24h
  (widened so a run can contain a sync at all); `brief-run` **is** present —
  five hits over seven days — so the denominator this entry calls load-bearing
  is satisfied and a silent overnight result is now readable rather than
  ambiguous; and the run that settled it was #12, not the 06:00 job this entry
  was waiting on, which has long since aged out of any window.
