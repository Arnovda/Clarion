...
**Prior last updated:** 2026-09-12 (THE SQL CONNECTORS ARE IN PRODUCTION — owner: *"Merge
...
**Prior last updated:** 2026-09-09 (A READABILITY GATE, NOT JUST A VALIDITY GATE — a
...
- Validation: backend `npm run check` clean; NEW `widget-readability.test.ts`
  ...
  **RE-VALIDATED AFTER THE REBASE ONTO MAIN (2026-09-13** — main had moved
  18 commits: ingestion phases 0–2, the SQL connectors, the reader fixes;
  four files conflicted and were merged by hand — the five `readability-*`
  signatures now sit beside main's sync/delta ones in both reader files):
  backend `npm run check` clean, full vitest **95 files / 932 passed / 4
  skipped** (the delta sidecar suite ran for real here with
  `deltalake==1.6.3` installed instead of skipping), **all TWELVE ratchets
  green** from the repo root (`lint-warehouse-scan` is main's twelfth),
  frontend `tsc` clean, vitest 7/62, touched files lint-clean, `next build`
  46/46 (`/dashboards` 64.8 kB), `prod-logs.yml` parses.
...
- **DRIVE-BY ON THE SAME PR: BOTH AUDIT GATES WENT RED UNDER EVERY BRANCH**
  ...
  `invite-email` + `signup-hardening` 16/16). Both gates exit 0 locally. **SUPERSEDED ON REBASE (2026-09-13)**: by the time PR #129 was rebased onto main, main's `bde5589` (2026-09-09, 21:13 UTC) had cleared the SAME two advisories independently with the same two bumps (`js-yaml ^4.3.2`, `nodemailer ^9.1.1`), so this branch's copy collapsed to an empty commit and was dropped; the fixes that ship are main's. The diagnosis stays recorded because the shape — a red gate on a PR that touched none of that code — is the reusable part.
- **NOT done, deliberately**: the renderer still cycles colours past 6
  ...
  (moot since the rebase: main's 2026-09-10 work took that slack itself —
  the baseline is 73 and the rebased tree reports 73).

**Prior last updated:** 2026-09-08 (ODOO-VIA-RAW-TABLES READINESS — doc only, no product
