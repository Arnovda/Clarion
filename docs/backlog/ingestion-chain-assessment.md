...
> **Status 2026-09-10 (fase 2, eerste slice).** Gebouwd op het HUIDIGE formaat, zodat de semantiek staat vóór het formaat wisselt: **B2** (`_clarion_synced_at` / `_clarion_deleted` op elke brontabel, full re-sync merget en markeert wat het niet zag, nieuwe `reconcile`-mode op sleutels, views verbergen verwijderde rijen en de technische kolommen), **B3** (per-entiteit checkpoints die bij aankomst worden gepersisteerd, een tijdbudget in de worker met een nette stop, vervolgruns tot de load klaar is), **B6** (`rows_total` + `last_status` per entiteit, de catalogus toont ze), én **de merge is een anti-join** in plaats van een window-functie (O(delta) geheugen; de oude vorm ging out-of-memory op 3M rijen onder een eerlijke limiet — zie §9). **B1 (DuckLake) is NIET gewisseld.** De PoC (§9) bevestigt de winst, maar bracht twee ontwerpvragen boven die een eigenaarsbeslissing vragen vóór de bouw: (a) de sync-worker heeft bewust géén databasecredentials (de orchestrator persisteert alles), terwijl DuckLake's catalogus in Postgres leeft — de writer zou een Postgres-rol nodig hebben, of een per-connectie DuckDB-bestandscatalogus die de leeskant op blob niet kan delen; (b) DuckLake's catalogustabellen zijn niet tenant-gescheiden — een `METADATA_SCHEMA` per tenant plus RLS, of één lake per tenant. Beide zijn beslisbaar, geen van beide is klein. Tot dan is de anti-join-merge op parquet het substraat, en het is nu een substraat dat in de jobcontainer past.
>
> **Status 2026-09-10 (fase 2, tweede slice — B1 beslist op meting).** De twee ontwerpvragen hierboven zijn NIET beantwoord maar **weggevallen**: DuckLake is niet het formaat voor nu, en Delta (dat al in productie draait voor de topics, met de catalogus in `_delta_log` op blob) heeft geen van beide vragen. De meting in §9 laat delta-rs' MERGE op 1,3–1,9 GB voor een delta van 10k rijen en >4 GB voor een tabel-tegen-tabel merge zien — geen writer voor een 1-GiB container — en DuckLake's MERGE vraagt een DuckDB die de legacy Node-binding niet meer krijgt. Gedaan: de topic-sidecar streamt (geen pandas, tellingen uit DuckDB, 6,7 GB → ~300 MB), de views hebben de soft-delete-firewall ook op `delta_scan`, de onderhoudsjob compacteert en vacuümt de topic-tabellen. **Niet gedaan, met reden:** de bronlaag naar Delta — zonder een merge die in de container past levert dat O(tabel) I/O plus een Python-venv in de sync-worker op, voor atomiciteit en time travel die de anti-join-op-parquet vandaag niet mist. Terugkomen op B1 na de `@duckdb/node-api`-migratie (die de query-engine sowieso verschuldigd is), en alleen als de O(tabel)-I/O op een echte tenant voelbaar wordt.
...
| **2 — Het tabelformaat** 🟡 *B2/B3/B6 gebouwd; B1 op meting beslist 2026-09-10: topic-sidecar streamt op Delta, bronlaag blijft parquet met de anti-join, DuckLake pas na de node-api-migratie — zie de statusnoten onder §3.3 en de PoC in §9* | DuckLake (of Delta-MERGE) voor bron én topics; soft-delete-conventie + `reconcile`-mode (B2); hervatbare loads (B3); één writer; Python-sidecar weg | Lost O(tabel)-merge, deletes, 30-min-grens, historie en de dubbele writer in één beweging op; alles daarna bouwt erop | 3–4 weken, incl. migratie van bestaande data (eenmalige re-sync per tenant is acceptabel — er zijn nog geen betalende klanten en één connector per tenant) |
...
- DuckLake v1.0 is vijf maanden oud. De aanbeveling om ernaar te gaan is een architectuurkeuze die een proof-of-concept tegen echte tenant-data verdient vóór fase 2 start: één EO-tenant, `TransactionLines`, een nachtelijke incrementele MERGE, gemeten looptijd en geheugengebruik in de jobcontainer.
  **PoC gedaan 2026-09-10, synthetisch (geen live tenant): `backend/scripts/poc-ducklake-merge.js`.** 3.000.000 rijen, 18 kolommen in EO-`TransactionLines`-vorm (208 MB parquet), één nachtelijke delta van 10.000 rijen (5.000 updates + 5.000 nieuwe), DuckDB 1.4.2, `threads=1`, `memory_limit=1200MB` (60% van een 2-GiB jobcontainer):

  | Strategie | Looptijd | Piek-RSS | Resultaat |
  |---|---|---|---|
  | A. huidige merge (`UNION ALL BY NAME` + `ROW_NUMBER() OVER (PARTITION BY key)`, hele bestand herschreven) | — | — | **Out of Memory** bij 1,1 GiB, ook met `preserve_insertion_order=false` |
  | A3. dezelfde merge zonder limiet | 8,8–9,5 s | 1,3 GB | 3.005.000 rijen, 258 MB |
  | A4. anti-join (`existing WHERE NOT EXISTS (delta) UNION ALL delta`) | 4,5 s | +150 MB | 3.005.000 rijen, 208 MB — **nu de writer** |
  | B. DuckLake, Postgres-catalogus: initiële load uit parquet | 3,8 s | +115 MB | |
  | B. DuckLake `MERGE INTO`, 10k delta | **0,39–0,45 s** | **+37 MB** | 3.005.000 rijen; 5 snapshots, time travel naar versie 1 werkt |
  | B. DuckLake soft-delete `UPDATE` over de hele tabel | 4,7 s | +10 MB | |
  | C. DuckLake, DuckDB-bestandscatalogus | idem B | idem B | |

  Twee bevindingen vielen er onbedoeld uit. (1) **De DuckDB-geheugenlimiet was in productie nooit gezet**: DuckDB 1.4 weigert `memory_limit='70%'` (parser error op `%`), en beide guardrails slikten dat op in een try/catch bedoeld voor "een oudere build zonder de setting" — elke sessie (backend, child runner, worker-writer) draaide op DuckDB's eigen default. Gerepareerd op dezelfde dag (percentage wordt vóór de SET omgerekend naar een absolute waarde uit `process.constrainedMemory()`), gepind met een test die de instelling terugleest uit een live sessie. (2) De huidige merge kan een tabel van deze grootte onder een eerlijke limiet helemaal niet mergen; de anti-join wel. DuckLake's `MERGE` is daar nog eens ~10× sneller en ~4× zuiniger, en is O(delta) in I/O waar de anti-join O(tabel) blijft — de architectuurkeuze staat; de twee ontwerpvragen (worker-credentials, tenant-scheiding van de catalogus) staan onder §3.3.

  **Tweede meting 2026-09-10, delta-rs (het `deltalake`-pakket dat de topic-sidecar draait): `backend/scripts/poc-delta-rs.py`.** Dezelfde 3M-rijen tabel (316 MB parquet mét `_row_hash`), deltalake 1.6.3, elke operatie in een eigen proces zodat de piek-RSS die van de operatie is; `taskset -c 0` = de CPU-blik van de 0,5-vCPU jobcontainer:

  | Operatie | Looptijd | Piek-RSS proces | Opmerking |
  |---|---|---|---|
  | Oude sidecar (deltalake 0.23.2 + pandas): 3M-refresh met diff | 133 s | **6,7 GB** | wat er tot vandaag in de 1-GiB jobs-worker draaide — kan daar alleen OOM-killed worden |
  | delta-rs streaming OVERWRITE, RecordBatchReader met defaults | 5–7 s | 1,4 GB | "streaming" is het niet: pyarrow's `iter_batches` leest vooruit (alleen lezen: 557 MB) en delta-rs buffert |
  | idem, per row group + `pre_buffer=False`, `write_batch_size` 8192, row groups 65k, 1 CPU | 7,1 s | **303 MB** | alleen lezen 142 MB — **dit is de nieuwe topic-writer** |
  | delta-rs `MERGE`, 10k delta in 3M, `streamed_exec=True`, 4 CPU's | 2,8 s | 1,9 GB | |
  | idem met `max_spill_size` 300/600 MB (DataFusion-geheugenpool) | — | — | `Resources exhausted` — de hash join spilt niet, hij faalt |
  | idem `streamed_exec=False`, 1 CPU | 5,1 s | **1,3 GB** | het beste geval, en nog boven de container |
  | delta-rs `MERGE` topic-vorm (3M nieuwe staat vs 3M, upd/ins/del) | — | **>4,3 GB, killed** | |
  | pyarrow full-outer join op (sleutel, hash) voor de tellingen | 1,9 s | 1,7 GB | verworpen; de tellingen komen nu uit DuckDB (gebonden, spilt) |
  | delta-rs soft-delete `UPDATE` (1/600 rijen, elk bestand geraakt) | 3,6 s | 911 MB | |
  | `optimize.compact` + `vacuum` | <0,1 s | 100 MB | |

  **Conclusie, en zij verandert B1.** delta-rs' MERGE is voor een tabel van deze grootte geen writer die in een 1-GiB container past — niet met de gestreamde executor, niet met de andere, niet met een geheugenpool (die faalt in plaats van te spillen). DuckLake's MERGE past wél, maar vraagt DuckDB 1.5.2, en de legacy Node-binding waar het hele platform op staat (`duckdb-async` 1.4.2) is voor het laatst voor 1.4.x uitgebracht: eerst een migratie van de query-engine-client naar `@duckdb/node-api`. Wat WEL in de container past is de anti-join van fase 2 in DuckDB (O(delta) geheugen, O(tabel) I/O, seconden op SMB-schaal) en een streaming Delta-overwrite. Dus: **de topic-sidecar is herschreven** (streaming overwrite, `_row_hash` en de wijzigingstellingen in DuckDB, geen pandas — 6,7 GB → ~300 MB op dezelfde tabel), **de bronlaag blijft parquet met de anti-join**, en DuckLake blijft de optie voor ná de node-api-migratie, als de O(tabel)-I/O ooit voelbaar wordt. Het topic-Delta krijgt compactie + vacuum in de wekelijkse onderhoudsjob (dat deed tot vandaag niets).
