...
| ID | Finding | Evidence |
|---|---|---|
| **P1-1** | **REMEDIATED 2026-09-01** (third wave-2 PR). The AI queues run at concurrency 2 with per-tenant single-flight (`jobs/tenantFairness.ts` — two TENANTS proceed in parallel, one tenant never takes both slots; a clashing job is pushed back to the delayed set with `DelayedError`), and a queued build now says where it stands (`buildsAhead` on `/bus-matrix/active`, spoken on the Build page). The reapers are LIVENESS-based (`services/reapers.ts` + migration 90): a run is reaped when its heartbeat goes quiet for 10 min — the sync orchestrator stamps `heartbeat_at` on its existing progress flush, `runSchemaProfiler` beats `profiling_heartbeat_at` on a 60s timer at its export boundary — not merely when it is old (reproduced red first: a healthy running sync started 31 min ago, active seconds ago, was failed by the verbatim old rule); a 4h ceiling backstops a wedged-but-beating process. The replica-pin claim was half stale: `RUN_SCHEDULERS=false` had already confined reapers to the API since 2026-07-27, and with liveness reaping the pin's remaining reason is gone — but max_replicas without a KEDA rule is inert on Container Apps, so scaling out is now an unblocked capacity decision (documented in `infra/variables.tf` + the provision workflow), not a silent knob. Original finding: both AI queues at global concurrency 1; worker pinned to one replica; age-only reapers. | `jobs/workers.ts`, `services/reapers.ts`, `jobs/tenantFairness.ts` |
| **P1-2** | **REMEDIATED 2026-09-01** (fourth wave-2 PR). All five limiters share ONE Redis-backed fixed-window store (`middleware/rateLimitStore.ts` — hand-rolled on the existing ioredis, no new dependency), so replicas count together instead of multiplying every limit; without Redis (dev/CI) it degrades to the per-process MemoryStore, and a Redis ERROR fails OPEN (a blip must not 429 the product). Authenticated traffic is keyed by TENANT via a VERIFIED token (`tenantOrIpKey` — verified, not decoded: an unverified decode would let a caller choose another tenant's bucket as a DoS lever), so a tenant can now be throttled individually and an office NAT no longer shares one IP budget. The brute-force surface gained a second limiter keyed by the ACCOUNT under attack (`accountKey`), closing the distributed-single-account hole the old comment conceded to. Proven live against real Redis: 5 wrong passwords → 401s, 6th → 429, with `rl:brute-ip:…`, `rl:brute-acct:acct:victim@…`, and tenant buckets `rl:compute:t:<id>` all observed in redis-cli. Original finding: per-IP, in-memory, per-replica — ~3× every published limit at max_replicas=3. | `middleware/rateLimitStore.ts`, `index.ts` |
...
| **P1-4** | **HCL WRITTEN 2026-09-02** (sixth wave-2 PR — write-and-stop by standing owner boundary: no workflow runs terraform, state lives on the owner's machine, so the apply and the restore rehearsal are the owner's acts; `docs/runbooks/disaster-recovery.md` §6 is the checklist). The substantive fix is the **Neo4j recovery point**: daily Azure Backup snapshots of the neo4j-data share (Recovery Services vault + policy + protected share, retention 30d) — snapshots not dumps, because Community edition has no online backup and Neo4j's WAL recovers from a crash-consistent snapshot like a power cut; plus 30-day share soft-delete on the storage account (blob soft-delete never covered shares — share deletion was unrecoverable). Two findings SHARPENED on verification: **Postgres HA cannot simply be switched on** — Azure doesn't support HA on the Burstable tier the platform runs (`pg_sku = B_Standard_B1ms`), so it ships as `pg_high_availability` (default off) with the ~10× cost and SKU prerequisite stated; and **Redis ephemerality was a decision, not an oversight** (recorded in the redis command comment since the Fase-2 split — Postgres is the source of truth, the reconciler re-registers repeatables, the P1-1 reapers bound orphan lifetime), so persistence ships as `redis_persistence_enabled` (default off, AOF-everysec on an Azure Files mount, trade-offs stated) and the deliberate posture is now a WRITTEN RPO instead of folklore. The RTO/RPO offer per store — including what may be quoted to a customer and only after the rehearsal boxes are ticked — is `docs/runbooks/disaster-recovery.md`. Original finding: every stateful dependency a SPOF; Postgres no HA block; Redis `--save '' --appendonly no`; Neo4j a 5 GiB file share with no backup policy — replication but no recovery point. | `infra/main.tf`, `infra/variables.tf`, `docs/runbooks/disaster-recovery.md` |
...
| **P1-6** | **REMEDIATED 2026-09-02** (seventh wave-2 PR — the last wave-2 item). The finding needed sharpening first: request LOG lines already carried tenantId (requestLogger since the beginning); what didn't was every METRIC (api_response_time_ms and all four ai_* metrics had the tenant in scope and dropped it), every worker FAILURE line and trackException, and — the substance — any aggregation surface. Now: tenantId rides every App Insights metric as a dimension and every worker failure log/exception; a per-tenant 24h rolling window (`services/tenantRequestStats.ts` — one Redis hash per hour, fields namespaced by tenant, replicas share via HINCRBY exactly like the P1-2 store; fail-open both directions, so no Redis means "no data" never fabricated zeros) feeds NEW error-rate/latency columns on `/admin/tenants` — which was already the sync-success and AI-cost-per-tenant surface (P1-5), so the ONE dashboard the finding asked for is the console the operator already uses: requests, 5xx count, avg and bucket-p95 per tenant beside syncs and AI spend. The three alerting items parked here by earlier passes shipped with it: `clarion-brute-force` (both brute limiters now log the load-bearing 'brute force limit hit' line on every refusal — `bruteLimitHandler`), `clarion-queue-depth` (`jobs/queueDepthMonitor.ts` sweeps BullMQ waiting counts every minute in the scheduler-owning process, ships a `queue_depth` metric and logs 'queue depth high' at ≥QUEUE_DEPTH_WARN), both wired into `.ops/alerts`/alerts.yml. The uptime web test is deferred AGAIN, this time with the decision recorded in `.ops/alerts`: an untestable az ARM blob inside a fail-loud control risks the working alert path; it is one 3-minute portal act (standard availability test → clarion-alerts action group). Proven live: 'queue depth monitor started' at boot, 5×401→2×429 with two 'brute force limit hit' WARN lines, and a real tenant's authed requests accumulating in `obs:req:*` in redis-cli. Original finding: nothing aggregates by tenant — no per-tenant error rate, latency, sync success or cost-to-serve. | `services/tenantRequestStats.ts`, `jobs/queueDepthMonitor.ts`, `routes/adminTenants.ts` |
| **P1-7** | **REMEDIATED 2026-09-01** (fifth wave-2 PR, taken early — small and closes a live deploy hazard). test.yml gained a `frontend-checks` job (`tsc --noEmit` + the frontend unit tests); the deploy gate waits on the whole Tests workflow, so a frontend type error now blocks migrate-sql and deploy. Reproduced red first: a deliberate `const x: number = 'string'` in a page compiled clean through `next build` (exit 0) while `tsc` caught it. **The app has its first frontend unit tests**: a vitest + Testing Library harness (`frontend/vitest.config.ts`, `frontend/tests/`) with 13 tests — the step-tree derivation and autoLabel (previously verified only by throwaway dry-run scripts), the one SQL formatter's never-throw and string-literal contracts, and a first component test (EmptyState). `ignoreBuildErrors` deliberately stays ON — flipping it changes Docker/render-gate build behaviour for no remaining safety benefit now the gate refuses the commit first (comment in next.config.mjs updated to the new truth). Drive-by: the `$postcss` override reference broke every `npm install` under npm 10 ($refs resolve only against `dependencies`); replaced with a literal + a keep-in-step note. Original finding: `ignoreBuildErrors: true`, no job type-checked the frontend, zero frontend tests. | `.github/workflows/test.yml`, `frontend/vitest.config.ts` |
...
4. **P1-1** Give the reapers an owner and heartbeat so the worker can scale past
   ...
   meanwhile. *(DONE 2026-09-01 — see the P1-1 row above; scale-out itself is
   an unblocked capacity decision, needing min-replicas or a KEDA rule.)*
5. **P1-2** Redis-backed rate limiting keyed on tenant and account.
   *(DONE 2026-09-01 — see the P1-2 row above.)*
6. **P1-4** Postgres zone-redundant HA, Redis persistence, scheduled Neo4j dump
   ...
   *(HCL + RTO/RPO doc DONE 2026-09-02 — see the P1-4 row above. The apply and
   the rehearsal are the owner's acts: `docs/runbooks/disaster-recovery.md` §6.)*
7. **P1-6** Stamp tenant id on every log line, metric and trace; one dashboard
   ...
   *(DONE 2026-09-02 — see the P1-6 row above; the "one dashboard" is
   /admin/tenants, which now carries all four per tenant. Wave 2 complete.)*
...
4. **P1-7** Frontend type-check into the deploy gate; first component tests.
   *(DONE 2026-09-01, pulled forward into wave 2 — see the P1-7 row above.)*
