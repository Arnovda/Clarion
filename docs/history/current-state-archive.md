...
Entries moved out of `CLAUDE.md` on 2026-09-01, **verbatim and unedited**.

They cover 2026-04 through 2026-08-24 and were 88% of that file. Nothing here is
summarised or dropped: this is the same text, in the same order, moved so the
live file could be read start to finish again — which its own maintenance rules
require and its size had made impossible.

**These entries are still binding.** Several record hard-won failures (the
context-read-above-its-provider bug, the mismatched-sample FK defect, the
role-flip that would have broken every login) whose lessons apply to code that
is live today. When something in the current file points "see the archive",
this is the file it means.

Newest first, same as the live file.
