...
| **2. Config & logging** | **2a:** central `config.ts`; the 3 frontend-URL aliases collapsed into `config.appUrl` (also fixed the ''-in-prod fallback bug); JWT secret + prod weak-secret guard centralised in `requireJwtSecret` (MFA/WebAuthn signing now covered too); CORS/PORT on config; `lint-scattered-config` ratchet. **2b:** pino sweep of runtime `console.*` (~220 sites) + `lint-no-console` ratchet. Remaining `process.env` reads migrate opportunistically. | 2a DONE 2026-07-12 · 2b DONE 2026-07-12 |
| **3. Backend structure** | `startSSE(res)` helper (≈12 sites); break the `connections.ts ↔ SyncOrchestrator` cycle (move `profilingProgressPct` to a service); consolidate the `SET LOCAL app.current_tenant` primitive; finish `reqDb` migration in `dashboards.ts`/`query.ts`; **inline `res.status(500)`→`next(err)`/throw** (moved from Phase 1: with `express-async-errors` now active these are a consistency/logging issue, not a crash risk, and the clean conversion — removing nested try/catch — fits the file sweeps here); delete dead code (`utils/storage.ts`, root `shared/types.ts`). *Ratchets: forbid new internal `await import()`; forbid raw `res.status(500)` in routes.* | **DONE 2026-07-12** |
| **4. Validation & contracts** | Attach the orphaned Zod schemas, then cover `products`/`semantic`/`dashboards`/`notebooks`/`pipelines`; shared DTO module + typed `api.get<T>` wrapper; migrate `Connection`/`DataProduct`/`WidgetSpec`/`DashboardSpec` to kill the live drift. *Ratchet: require `validate()` on new mutating routes.* | **4a DONE 2026-07-12** (validation: 15→39 routes covered, all 6 orphaned schemas corrected-then-attached, `lint-validate-coverage` baseline 166 lower-only) · **4b DONE 2026-07-13** — canonical contract as two byte-identical copies (`backend/src/shared/contract.ts` + `frontend/lib/contract.ts`, `lint-contract-sync` gate) instead of an npm package: the frontend Docker build context is `frontend/`-only, so a `file:` package would have forced build-context + deploy changes for a types-only module. WidgetSpec/DashboardSpec/FilterSpec drift resolved (frontend superset won), ConnectionDto/DataProductDto canonicalised for incremental adoption, typed `apiGet<T>`/`apiPost<T>` helpers + 3 exemplar conversions in dashboards/page.tsx. |
| **5. Frontend quality** | **5a DONE 2026-07-12** (safe subset, gated on typecheck + prod build): `lib/sse.ts` `streamSSE` helper — all 9 copy-paste reader loops migrated, abort-on-unmount added to the 3 sites that leaked streams (dashboards batch-execute, query think/repair); `lib/storage.ts` — all 34 raw localStorage calls centralised; dead code removed (`Nav.tsx`, `ui/NotificationBell.tsx`, deps `react-markdown`/`remark-gfm`/`canvg`). **5b (deferred — needs visual verification, no frontend tests exist):** fix 23 `exhaustive-deps` disables; split `sources`/`pipelines`/`RelationshipCanvas`; `useReducer` for 48-useState pages; adopt-or-delete `ui/` primitives. | 5a done · 5b deferred |
| **6. Service layer** | Split `routes/products.ts` (4,561 lines, 57 handlers) into `routes/products/` (10 focused modules + composition index) + `services/productWarehouse.ts`. Bodies moved byte-for-byte; **route-registration order proven byte-identical** via a router-stack diff against the pre-split file (57/57, empty diff). Two routes live in build.ts purely for order preservation (commented). | **DONE 2026-07-12** |
...
- **2026-07-12 — Phases 4a + 6 complete.** 4a: validation 15→39 routes; all six orphaned schemas had drifted from route reality and were corrected before attaching; 24 new schemas; `lint-validate-coverage` gate (baseline 166 unvalidated, lower-only). 6: products.ts split into `routes/products/` (catalog/core/design/tables/refine/kpis/build/refineChat/cells + shared + index) with byte-identical route order (router-stack diff proof) + `services/productWarehouse.ts`. **Remaining:** 4b (shared contract package `packages/contract` — touches Docker builds + deploy workflow, do as a dedicated pass with local docker-build verification) · 5b (frontend visual-risk items — needs a session with browser preview verification) · opportunistic: adopt `setTenantContext` in auth.ts/mfa/webauthn/busMatrixBuilder inline SET LOCALs; lower the dynamic-import baseline (94) as modules migrate to static imports; extend validate coverage below 166.
- **2026-07-12 — Phases 3 + 5a complete.** Phase 3: cycle `connections↔SyncOrchestrator` broken (profilingProgressPct → services/, both directions static); `startSSE` helper (services/sse.ts) adopted at all 12 sites; reqDb audit of dashboards/query found nothing left to convert (all remaining semanticDb uses are transactions); error-leaking inline 500s → `throw err` (products ×6, ingestion, connections) with `// deliberate-500` markers for the two intentional ones; dead `utils/storage.ts` + root `shared/types.ts` deleted; `setTenantContext` primitive (db/tenantContext.ts) consolidates tenantQuery/tenantScopedWrite/refreshTokenService; ratchets 4+5 (`lint-no-internal-dynamic-import` baseline=94 lower-only, `lint-no-inline-500`). Phase 5a: see the phase row. Frontend `npm run check` was red locally under newer TS (3 MapIterator/implicit-any errors in DatabaseTree.tsx) — fixed. Remaining inline SET LOCALs (auth.ts register, mfa/webauthn services, busMatrixBuilder) are candidates for a later setTenantContext adoption pass.
- **2026-07-12 — Phase 2a complete** (commit `fb60e63`, CI green incl. the new scattered-config ratchet). Note the constraint set by the user: **no change may increase app running cost** — all phases are code-only; the only marginal cost anywhere is CI minutes for the re-enabled test gate.
