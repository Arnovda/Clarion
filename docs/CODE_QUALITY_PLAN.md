...
| **1. Crash & AI safety** | `express-async-errors` (or `asyncHandler`) + process-level `unhandledRejection`/`uncaughtException`; replace 13 inline `res.status(500)` with `next(err)`; Zod-gate AI outputs (star schema, dashboard spec, schema draft) before persistence | Not started |
...
| **3. Backend structure** | `startSSE(res)` helper (≈12 sites); break the `connections.ts ↔ SyncOrchestrator` cycle (move `profilingProgressPct` to a service); consolidate the `SET LOCAL app.current_tenant` primitive; finish `reqDb` migration in `dashboards.ts`/`query.ts`; delete dead code (`utils/storage.ts`, root `shared/types.ts`). *Ratchet: forbid new internal `await import()`.* | Not started |
