...
const DB_PATH = path.resolve(__dirname, '../../data/sample.db');
...
const insertLine = db.prepare(`
  INSERT INTO order_lines (order_id, product_id, quantity, unit_price, discount_pct, line_total)
  VALUES (?, ?, ?, ?, ?, ?)
`);

const insertInvoice = db.prepare(`
  INSERT INTO invoices (order_id, customer_id, invoice_number, invoice_date, due_date,
                        total_excl_vat, vat_amount, total_incl_vat, status, paid_at)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`);
  ...
  for (let customerId = 1; customerId <= 40; customerId++) {
    ...
    for (let o = 0; o < numOrders; o++) {
      ...
      for (let l = 0; l < numLines; l++) {
        ...
        insertLine.run(orderId, productIdx + 1, quantity, product.unit_price, discount, lineTotal);
      }
