 ...
 */
/**
 * The raw SOURCE layer is a curator surface (admin/analyst — the roles
 * that may see SQL). The frontend hides the toggle from viewers, but until
 * 2026-09-06 the API honoured `dataLayer:'source'` from any role, so a
 * viewer's hand-made request reached source tables past the product
 * layer's curated view. A viewer's request for 'source' is IGNORED, not
 * refused: they get the product layer when one exists and the source
 * layer only where it is the sole option — exactly what the UI gives them.
 */
const SOURCE_LAYER_ROLES = new Set(['admin', 'analyst']);
export function layerForRole(req: Request, requested: 'product' | 'source' | undefined): 'product' | 'source' | undefined {
  if (requested === 'source' && !SOURCE_LAYER_ROLES.has(req.user!.role)) return undefined;
  return requested;
}
