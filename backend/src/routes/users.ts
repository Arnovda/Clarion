...
router.post('/invite', requireRole('admin'), validate(inviteUserSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const invited = await inviteUser(db, {
      tenantId: req.user!.tenantId, email, displayName, role, inviter: req.user!.email,
    });
    if (invited.kind === 'refused') {
      res.status(invited.status).json({ ok: false, error: invited.error });
      return;
    }
    const { user, inviteUrl, emailed } = invited;
