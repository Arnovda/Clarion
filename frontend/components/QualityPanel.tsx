...
function rag(score: number | null): string {
  if (score === null) return 'bg-white/40 text-on-surface-variant';
  if (score >= 0.9)   return 'bg-emerald-500/15 text-emerald-600 border border-emerald-500/20';
  if (score >= 0.7)   return 'bg-amber-500/15 text-amber-600 border border-amber-500/20';
...
function statusBadge(s: string | null) {
  ...
  const cls =
    s === 'PASS'           ? 'bg-emerald-500/15 text-emerald-600 border border-emerald-500/20' :
    s === 'WARNING'        ? 'bg-amber-500/15 text-amber-600 border border-amber-500/20'     :
    s === 'NOT_CONFIGURED' ? 'bg-white/40 text-on-surface-variant border border-white/40'     :
...
function Sparkbar({ data }: { data: Array<{ count: number }> }) {
  if (!data?.length) return <span className="text-on-surface-variant text-xs">—</span>;
...
function Panel({
  ...
  return (
    <div className="glass-card rounded-2xl overflow-hidden">
      <div
        className="flex items-center justify-between px-5 py-4 cursor-pointer select-none hover:bg-white/40"
        onClick={onToggle}
      >
      ...
      {open && <div className="border-t border-white/40">{children}</div>}
    </div>
  );
}
...
function AddRuleForm({
  ...
  const inp = 'bg-white/60 border border-white/80 rounded-xl px-4 py-2.5 text-sm w-full focus:outline-none focus:ring-2 focus:ring-cyan-400/30';
  ...
  return (
    <div className="p-5 bg-surface-container border-t border-white/40 grid grid-cols-2 gap-3">
      <div className="col-span-2">
        <label className="block text-xs text-on-surface-variant mb-1">Rule name *</label>
      ...
      <div>
        <label className="block text-xs text-on-surface-variant mb-1">Dimension</label>
        <select className={inp} value={form.dimension} onChange={(e) => set('dimension', e.target.value)}>
          {DIMENSIONS.map((d) => <option key={d}>{d}</option>)}
        </select>
      </div>
      <div>
        <label className="block text-xs text-on-surface-variant mb-1">Rule type</label>
      ...
      {needsField && (
        <div>
          <label className="block text-xs text-on-surface-variant mb-1">Field *</label>
      ...
      <div>
        <label className="block text-xs text-on-surface-variant mb-1">Pass threshold</label>
      ...
      {form.rule_type === 'range' && (
        <>
          <div>
            <label className="block text-xs text-on-surface-variant mb-1">Min value</label>
            <input className={inp} type="number" value={form.range_min}
              onChange={(e) => set('range_min', e.target.value)} placeholder="e.g. 0" />
          </div>
          <div>
            <label className="block text-xs text-on-surface-variant mb-1">Max value</label>
      ...
      {form.rule_type === 'format' && (
        <div className="col-span-2">
          <label className="block text-xs text-on-surface-variant mb-1">Regex pattern</label>
      ...
      {needsFreshness && (
        <>
          <div>
            <label className="block text-xs text-on-surface-variant mb-1">Date field *</label>
          ...
          <div>
            <label className="block text-xs text-on-surface-variant mb-1">Max age (hours)</label>
      ...
      {needsCustom && (
        <div className="col-span-2">
          <label className="block text-xs text-on-surface-variant mb-1">
            SQL — must return failing record IDs as <code className="bg-white/40 px-1 rounded">record_id</code>
          </label>
      ...
      <div>
        <label className="block text-xs text-on-surface-variant mb-1">Owner</label>
      ...
      <div className="col-span-2 flex gap-2 justify-end">
        <button onClick={onCancel}
          className="px-3 py-1.5 text-sm rounded-xl bg-white/60 border border-white/80 hover:bg-white/80">
          Cancel
        </button>
        <button
          onClick={() => onSave(form)}
          disabled={!form.rule_name.trim()}
          className="px-3 py-1.5 text-sm rounded-xl gradient-primary text-on-primary shadow-glow-primary hover:shadow-glow-teal-md disabled:opacity-40"
        >
          {saveLabel}
        </button>
      </div>
...
function FailureDrawer({ failure, onClose, onStatusChange }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end backdrop-blur-sm" onClick={onClose}>
      <div className="w-96 bg-surface-container-lowest/95 backdrop-blur-xl shadow-ambient-lg h-full border-l border-white/40 p-6 overflow-y-auto"
        onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <span className="font-semibold text-on-surface">Failure detail</span>
          <button onClick={onClose} className="text-on-surface-variant hover:text-on-surface text-lg">✕</button>
        </div>
        <dl className="space-y-3 text-sm">
          <div><dt className="text-on-surface-variant text-xs">Rule</dt><dd className="font-medium">{failure.rule_name}</dd></div>
          <div><dt className="text-on-surface-variant text-xs">Field</dt><dd>{failure.field_name ?? '—'}</dd></div>
          <div><dt className="text-on-surface-variant text-xs">Actual value</dt><dd className="font-mono bg-white/40 rounded px-2 py-1">{failure.actual_value ?? '—'}</dd></div>
          <div><dt className="text-on-surface-variant text-xs">Expected</dt><dd>{failure.expected_description ?? '—'}</dd></div>
          <div><dt className="text-on-surface-variant text-xs">First detected</dt><dd>{new Date(failure.first_detected).toLocaleString()}</dd></div>
          <div>
            <dt className="text-on-surface-variant text-xs mb-1">Status</dt>
...
export default function QualityPanel({
  ...
  // ─── Render ───────────────────────────────────────────────────────────────
  return (
    <div className="flex-1 min-h-0 overflow-y-auto bg-surface-container-low">
      <div className="p-6 space-y-4 max-w-6xl">
        ...
        <div className="glass-card rounded-2xl px-5 py-4">
          <div className="flex items-start gap-6 flex-wrap">
            <div className="flex-1 min-w-60">
              <p className="text-sm font-semibold text-on-surface mb-0.5">Business Key</p>
            ...
            <div className="flex items-end gap-2">
              <div>
                <label className="block text-xs text-on-surface-variant mb-1">
                  Column
                  {bkSettings?.user_bk && (
                    <span className="ml-2 px-1.5 py-0.5 bg-cyan-500/15 text-cyan-600 border border-cyan-500/20 rounded text-[10px] font-semibold">USER-SET</span>
                  )}
                  {!bkSettings?.user_bk && bkSettings?.suggested_bk && (
                    <span className="ml-2 px-1.5 py-0.5 bg-white/40 text-on-surface-variant border border-white/40 rounded text-[10px] font-semibold">AUTO-DETECTED</span>
                  )}
                </label>
                <select
                  className="bg-white/60 border border-white/80 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-400/30 min-w-40"
                  value={bkDraft}
              ...
              {bkDirty && (
                <button
                  onClick={saveBk}
                  disabled={bkSaving}
                  className="px-3 py-1.5 text-sm rounded-xl gradient-primary text-on-primary shadow-glow-primary hover:shadow-glow-teal-md disabled:opacity-50"
                >
              ...
              {!bkDirty && bkSettings?.user_bk && (
                <button
                  ...
                  className="px-2.5 py-1.5 text-xs rounded-xl bg-white/60 border border-white/80 text-on-surface-variant hover:bg-white/80"
        ...
        <div className="glass-card rounded-2xl overflow-hidden">
          <div className="px-5 py-4 flex items-center justify-between">
            ...
            <div className="flex flex-col items-end gap-1">
              <button
                onClick={runProfile}
                disabled={profiling}
                className="px-3 py-1.5 text-sm rounded-xl gradient-primary text-on-primary shadow-glow-primary hover:shadow-glow-teal-md disabled:opacity-50 flex items-center gap-1.5"
              >
                {profiling ? (
          ...
          {summary && (
            <div className="border-t border-white/40 px-5 pb-5">
              ...
              {sortedHistory.length > 1 && (
                <div className="mt-5">
                  <p className="text-xs text-on-surface-variant mb-2 font-medium">90-day score trend</p>
          ...
          {!summary && (
            <div className="border-t border-white/40 px-5 py-8 text-center text-on-surface-variant text-sm">
        ...
        <Panel
          ...
          action={
            <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
              <input
                className="bg-white/60 border border-white/80 rounded-xl px-4 py-2.5 text-xs w-32 focus:outline-none focus:ring-2 focus:ring-cyan-400/30"
                placeholder="Search fields…"
                value={fieldSearch}
                onChange={(e) => setFieldSearch(e.target.value)}
              />
              <select
                className="bg-white/60 border border-white/80 rounded-xl px-4 py-2.5 text-xs focus:outline-none focus:ring-2 focus:ring-cyan-400/30"
        ...
        >
          {!fields.length ? (
            <p className="px-5 py-6 text-sm text-on-surface-variant">No field profile data yet.</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-surface-container text-xs text-on-surface-variant border-b border-white/40">
                ...
                <tbody>
                  {filteredFields.map((f) => {
                    ...
                    const uniqueColour   = f.distinct_pct >= 0.9 ? 'text-emerald-600' : f.distinct_pct >= 0.5 ? 'text-on-surface' : 'text-on-surface-variant';
                    return (
                      <React.Fragment key={f.id}>
                        <tr
                          className={`border-b border-white/40 cursor-pointer hover:bg-white/40 transition-colors ${highlighted ? 'ring-1 ring-inset ring-cyan-400/30 bg-cyan-50/20' : ''}`}
                          onClick={() => setExpandedField(isExp ? null : f.id)}
                        >
                          <td className="px-4 py-2.5 font-medium text-on-surface">
                            <div className="flex items-center gap-1.5">
                              <span className="text-on-surface-variant">{isExp ? '▾' : '▸'}</span>
                              {f.field_name}
                              {isBk && <span className="px-1 py-0 bg-cyan-500/15 text-cyan-600 border border-cyan-500/20 text-[10px] rounded font-semibold">BK</span>}
                            </div>
                          </td>
                          <td className="px-4 py-2.5 text-on-surface-variant text-xs font-mono">{f.data_type || '—'}</td>
                          ...
                          <td className="px-4 py-2.5 text-right font-mono text-xs text-on-surface-variant">{f.min_value ?? '—'}</td>
                          <td className="px-4 py-2.5 text-right font-mono text-xs text-on-surface-variant">{f.max_value ?? '—'}</td>
                          <td className="px-4 py-2.5 text-right font-mono text-xs text-on-surface-variant">{fmt(f.mean_value)}</td>
                          <td className="px-4 py-2.5 text-right font-mono text-xs text-on-surface-variant">{fmt(f.median_value)}</td>
                          ...
                          <tr className="bg-surface-container">
                            <td colSpan={9} className="px-6 py-4">
                              <div className="grid grid-cols-2 gap-6">
                                <div>
                                  <p className="text-xs font-medium text-on-surface-variant mb-2">Top values</p>
                                  {f.top_values?.length ? (
                                    <div className="space-y-1">
                                      {f.top_values.map((tv, i) => (
                                        <div key={i} className="flex items-center gap-2 text-xs">
                                          <span className="w-32 truncate text-on-surface font-mono">{tv.value}</span>
                                          <div className="flex-1 bg-white/40 rounded-full h-1.5">
                                            <div className="bg-cyan-500 h-1.5 rounded-full" style={{ width: `${tv.pct * 100}%` }} />
                                          </div>
                                          <span className="text-on-surface-variant w-10 text-right">{pct(tv.pct)}</span>
                                        </div>
                                      ))}
                                    </div>
                                ...
                                <div>
                                  <p className="text-xs font-medium text-on-surface-variant mb-2">Distribution</p>
                                  {f.histogram?.length ? (
                                    ...
                                    <div className="flex flex-wrap gap-1.5">
                                      {f.top_values?.map((tv, i) => (
                                        <span key={i} className="px-1.5 py-0.5 bg-cyan-500/15 text-cyan-600 text-xs rounded font-mono">
          ...
          action={
            <div className="flex gap-2">
              <button
                ...
                className="px-2.5 py-1 text-xs rounded-xl bg-white/60 border border-white/80 text-on-surface hover:bg-white/80 disabled:opacity-40 font-medium flex items-center gap-1"
              >
                {evaluating
              ...
              <button
                className="px-2.5 py-1 text-xs rounded-xl gradient-primary text-on-primary shadow-glow-primary hover:shadow-glow-teal-md font-medium"
        ...
        >
          ...
          {!rules.length && !addingRule ? (
            <p className="px-5 py-6 text-sm text-on-surface-variant">No rules configured yet. Add a rule to start monitoring data quality.</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  ...
                  {rules.map((rule) => {
                    ...
                    return (
                      <React.Fragment key={rule.id}>
                        <tr
                          className={`border-b border-white/40 cursor-pointer hover:bg-white/40 ${fieldMatch ? 'ring-1 ring-inset ring-cyan-400/30 bg-cyan-50/20' : ''}`}
                          onClick={() => setExpandedRule(isExp ? null : rule.id)}
                        >
                          <td className="px-4 py-2.5 font-medium text-on-surface">
                            <span className="text-on-surface-variant mr-1.5">{isExp ? '▾' : '▸'}</span>
                            {rule.rule_name}
                          </td>
                          <td className="px-4 py-2.5">
                            <span className="px-1.5 py-0.5 bg-white/40 text-on-surface border border-white/40 rounded text-xs">{rule.dimension}</span>
                          </td>
                          <td className="px-4 py-2.5 text-xs font-mono text-on-surface-variant">{rule.rule_type}</td>
                          <td className="px-4 py-2.5 text-xs text-on-surface-variant max-w-[8rem] truncate">
                          ...
                          <td className="px-4 py-2.5 text-center" onClick={(e) => e.stopPropagation()}>
                            <button
                              ...
                              className={`w-8 h-4 rounded-full transition-colors ${rule.is_active ? 'bg-cyan-500' : 'bg-white/40'}`}
                          ...
                          <td className="px-4 py-2.5 text-right" onClick={(e) => e.stopPropagation()}>
                            <div className="flex items-center justify-end gap-2">
                              <button
                                ...
                                className="text-on-surface-variant hover:text-cyan-600 text-xs transition-colors"
                                title="Edit rule"
                              >✎</button>
                              <button
                                onClick={() => deleteRule(rule.id)}
                                className="text-on-surface-variant hover:text-red-500 text-xs transition-colors"
                                title="Delete rule"
                              >✕</button>
                            </div>
                          </td>
                            ...
                            <td colSpan={9} className="px-6 py-4">
                              <div className="grid grid-cols-2 gap-4 text-sm">
                                <dl className="space-y-1.5">
                                  {rule.description && (
                                    <div><dt className="text-xs text-on-surface-variant">Description</dt><dd>{rule.description}</dd></div>
                                  )}
                                  <div><dt className="text-xs text-on-surface-variant">Pass threshold</dt><dd>{pct(rule.pass_threshold)}</dd></div>
                                  {rule.owner_name && (
                                    <div><dt className="text-xs text-on-surface-variant">Owner</dt><dd>{rule.owner_name}</dd></div>
                                  )}
                                </dl>
                                <div>
                                  <button
        ...
        <Panel
          ...
          action={
            <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
              ...
              <input
                className="bg-white/60 border border-white/80 rounded-xl px-4 py-2.5 text-xs w-28 focus:outline-none focus:ring-2 focus:ring-cyan-400/30"
        ...
        >
          {!failures.length ? (
            <p className="px-5 py-6 text-sm text-on-surface-variant">No failures found for the selected filters.</p>
          ) : (
            <>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {failures.map((f) => (
                      <tr
                        key={f.id}
                        className="border-b border-white/40 cursor-pointer hover:bg-white/40"
                        onClick={() => setDrawerFailure(f)}
                      >
                        <td className="px-4 py-2.5 text-on-surface">{f.rule_name}</td>
                        <td className="px-4 py-2.5 text-on-surface-variant font-mono text-xs">{f.field_name ?? '—'}</td>
                        <td className="px-4 py-2.5 font-mono text-xs text-on-surface max-w-[12rem] truncate">{f.actual_value ?? '—'}</td>
                        <td className="px-4 py-2.5 text-xs text-on-surface-variant max-w-[16rem] truncate">{f.expected_description ?? '—'}</td>
                        <td className="px-4 py-2.5 text-xs text-on-surface-variant">{new Date(f.first_detected).toLocaleDateString()}</td>
                        <td className="px-4 py-2.5 text-center">
                          <span className={`px-1.5 py-0.5 rounded text-xs ${
                            f.status === 'resolved'       ? 'bg-emerald-500/15 text-emerald-600 border border-emerald-500/20' :
                            f.status === 'in_remediation' ? 'bg-blue-500/15 text-blue-600 border border-blue-500/20' :
                            f.status === 'known'          ? 'bg-amber-500/15 text-amber-600 border border-amber-500/20' :
                                                            'bg-red-500/15 text-red-600 border border-red-500/20'
                          }`}>{f.status}</span>
                        </td>
                      </tr>
              ...
              <div className="flex items-center justify-between px-4 py-3 border-t border-white/40 text-xs text-on-surface-variant">
                ...
                <div className="flex gap-1">
                  <button disabled={failPage === 1} onClick={() => setFailPage((p) => p - 1)}
                    className="px-2 py-1 rounded-xl bg-white/60 border border-white/80 disabled:opacity-40 hover:bg-white/80">← Prev</button>
                  <span className="px-2 py-1">Page {failPage} / {failPages}</span>
                  <button disabled={failPage >= failPages} onClick={() => setFailPage((p) => p + 1)}
                    className="px-2 py-1 rounded-xl bg-white/60 border border-white/80 disabled:opacity-40 hover:bg-white/80">Next →</button>
                </div>
              </div>
            </>
          )}
      ...
      {/* Toast */}
      {toast && (
        <div className={`fixed bottom-6 right-6 z-50 px-4 py-3 rounded-xl shadow-lg text-sm font-medium flex items-center gap-2 backdrop-blur-sm ${
