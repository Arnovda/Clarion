...
import { Sparkles, X, ArrowUp, AlertTriangle, Layers, Database, Loader2 } from 'lucide-react';
...
export default function AskAIPanel({
  ...
  // - product mode: the product's connection
  // - general mode: first connection that has any product, else first connection
  ...
  return (
    <>
      ...
      <aside
        role="dialog"
        aria-label="Ask AI"
        className="fixed top-0 right-0 bottom-0 w-full sm:w-[480px] bg-raised border-l border-line z-50 flex flex-col shadow-3 ai-panel-enter"
      >
        {/* Header */}
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          {thinking && (
            <ThinkingRow phase={thinkingPhase} text={thinkingText} />
          )}
        </div>
        ...
        <footer className="shrink-0 border-t border-line px-4 py-3 bg-raised">
          <div className={cn(
            ...
            <textarea
              ...
              onKeyDown={onKeyDown}
              rows={1}
              placeholder={product ? `Ask about ${product.name}…` : 'Ask across all products…'}
              disabled={thinking || !resolvedConnectionId}
            ...
            <button
              type="button"
              onClick={() => send()}
              disabled={!input.trim() || thinking || !resolvedConnectionId}
              aria-label="Send"
              className={cn(
                'w-7 h-7 rounded-md flex items-center justify-center shrink-0 transition-all',
                input.trim() && !thinking && resolvedConnectionId
    ...
    return (
      <div className="text-center py-12 px-4">
        ...
        <p className="text-[12px] text-muted mt-1 leading-relaxed">
          Build one with “Prepare my data” to ask AI about it here.
        </p>
      </div>
    );
  }
  return (
    <div className="py-2">
      ...
      <p className="text-[13.5px] text-ink-2 leading-relaxed mb-4">
        Ask anything about {productMode ? 'this product' : 'your organized data'}.
        I&rsquo;ll generate SQL, run it on the warehouse, and answer in plain language.
      </p>
...
function MessageRow({ msg }: { msg: PanelMessage }) {
  if (msg.role === 'user') {
    return (
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center gap-1.5">
        ...
        {typeof msg.confidence === 'number' && !msg.blocked && !msg.error && (
...
function ThinkingRow({ phase, text }: { phase: string; text: string }) {
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center gap-1.5">
        <Sparkles className="w-3 h-3 text-ocean ai-sparkle" strokeWidth={1.75} />
        <span className="text-[10px] font-mono tracking-[0.14em] uppercase text-ocean">AI is thinking</span>
      ...
      <div className="rounded-md px-3.5 py-2.5 text-[13px] leading-relaxed bg-softer border border-line text-ink-2 ai-sheen">
        ...
        {text
          ...
          : <p className="text-muted italic">Loading context and reasoning…</p>
        }
      </div>
    </div>
  );
