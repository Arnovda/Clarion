...
export default function ProfilePage() {
  ...
  async function handleSaveName() {
    ...
    catch { alert('Failed to update name'); } finally { setSavingName(false); }
  }
  ...
  return (
    <AppShell title="Profile" showSearch={false}>
      {loading ? (
        <div className="p-8 text-on-surface-variant">Loading...</div>
      ) : !profile ? (
        <div className="p-8 text-on-surface-variant">Could not load profile</div>
      ) : (
        <div className="max-w-2xl mx-auto px-6 py-8 space-y-6">
          {/* Avatar + name */}
          <div className="bg-surface-container-lowest rounded-2xl shadow-ambient p-6 flex items-center gap-5">
            <label className="relative cursor-pointer group" title="Click to change avatar">
              {profile.avatar_url ? (
                <img src={profile.avatar_url} alt="" className="w-16 h-16 rounded-full object-cover" />
              ) : (
                <div className="w-16 h-16 rounded-full bg-primary/10 text-primary flex items-center justify-center text-2xl font-headline font-bold">
              ...
              <input type="file" accept="image/*" className="hidden" onChange={async (e) => {
                ...
                if (file.size > 375000) { alert('Image too large (max 375KB)'); return; }
                const reader = new FileReader();
                reader.onload = async () => { try { await api.post('/users/profile/avatar', { avatar: reader.result }); loadProfile(); } catch { alert('Failed to upload avatar'); } };
              ...
              {editingName ? (
                <div className="flex items-center gap-2">
                  <input type="text" value={nameInput} onChange={(e) => setNameInput(e.target.value)} autoFocus
                    className="px-2.5 py-1 rounded-lg text-body-md bg-surface-container-low text-on-surface focus:outline-none focus:ring-1 focus:ring-cyan-400" />
                  <button onClick={handleSaveName} disabled={savingName}
                    className="text-label-md text-secondary font-semibold hover:text-primary">{savingName ? 'Saving...' : 'Save'}</button>
                  <button onClick={() => { setEditingName(false); setNameInput(profile.display_name); }}
                    className="text-label-md text-on-surface-variant hover:text-on-surface">Cancel</button>
                </div>
              ) : (
                <div className="flex items-center gap-2">
                  <h2 className="font-headline text-headline-sm font-bold text-on-surface">{profile.display_name}</h2>
                  <button onClick={() => setEditingName(true)} className="text-label-md text-secondary hover:text-primary">Edit</button>
                </div>
              )}
          ...
          <div className="bg-surface-container-lowest rounded-2xl shadow-ambient overflow-hidden">
            <div className="px-6 py-4 flex items-center justify-between ghost-border-b">
              <h2 className="text-title-md text-on-surface">Password</h2>
              <button onClick={() => { setShowPassword(!showPassword); setPwMessage(null); }}
                className="text-label-lg text-secondary font-semibold hover:text-primary transition-colors">
            ...
            {showPassword && (
              <form onSubmit={handleChangePassword} className="px-6 py-5 space-y-4">
                <div>
                  <label className="block text-label-lg text-on-surface mb-1.5">Current password</label>
                ...
                {pwMessage && (
                  <div className={`p-3 rounded-xl text-body-sm ${pwMessage.type === 'success' ? 'bg-green-50 text-green-700' : 'bg-error-container/30 text-error'}`}>
                    {pwMessage.text}
                  </div>
                )}
              </form>
